...
final class AffirmationRepositoryTests: XCTestCase {
    ...
    // not deallocated between `setUp` and the first assertion. Before this
    // fix, `PersistenceController(inMemory:)` was a temporary — its `.container`
    // was pulled off and the controller went away, which took the Core Data
    // stack down with it and left `testFetchByCategory` SIGSEGV-ing in CI.
