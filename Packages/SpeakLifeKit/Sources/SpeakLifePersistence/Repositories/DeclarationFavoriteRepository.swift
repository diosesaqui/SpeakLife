...
public protocol DeclarationFavoriteRepositoryProtocol: Repository where Entity == DeclarationFavoriteEntry {
...
public final class DeclarationFavoriteRepository: DeclarationFavoriteRepositoryProtocol {
    ...
    // MARK: - Create
    public func create(_ entity: DeclarationFavoriteEntry) async throws {
    ...
    // MARK: - Update
    public func update(_ entity: DeclarationFavoriteEntry) async throws {
        try await context.perform {
            entity.lastModified = Date()
            try self.context.save()
        }
    ...
    // MARK: - Delete
    public func delete(_ entity: DeclarationFavoriteEntry) async throws {
        try await context.perform {
    ...
    // MARK: - Fetch
    public func fetch(predicate: NSPredicate? = nil) async throws -> [DeclarationFavoriteEntry] {
        try await context.perform {
    ...
    // MARK: - Fetch by ID
    public func fetchById(_ id: UUID) async throws -> DeclarationFavoriteEntry? {
    ...
    public func findByDeclarationId(_ declarationId: String) async throws -> DeclarationFavoriteEntry? {
    ...
    public func fetchByCategory(_ category: String) async throws -> [DeclarationFavoriteEntry] {
        let predicate = NSPredicate(format: "category == %@", category)
        return try await fetch(predicate: predicate)
    }
    
    // MARK: - Fetch by Content Type
    public func fetchByContentType(_ contentType: String) async throws -> [DeclarationFavoriteEntry] {
        let predicate = NSPredicate(format: "contentType == %@", contentType)
        return try await fetch(predicate: predicate)
    }
    
    // MARK: - Delete by Declaration ID
    public func deleteByDeclarationId(_ declarationId: String) async throws {
    ...
    // MARK: - Observe All
    public func observeAll() -> AnyPublisher<[DeclarationFavoriteEntry], Never> {
        let request = DeclarationFavoriteEntry.fetchRequest()
        request.sortDescriptors = [
                NSSortDescriptor(keyPath: \DeclarationFavoriteEntry.createdAt, ascending: false),
                // Total order, same reason as AudioFavoriteRepository.
    ...
    public func createFromDeclaration(_ declaration: Declaration) async throws -> DeclarationFavoriteEntry {
    ...
    public func toDeclaration(_ entry: DeclarationFavoriteEntry) -> Declaration {
        return Declaration(
            text: entry.text ?? "",
            book: entry.book,
