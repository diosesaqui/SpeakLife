...
public final class ProgressSyncStore {

    public static let shared = ProgressSyncStore()

    // MARK: - Event kinds

    public enum Kind {
        public static let dayCompletion = "dayCompletion"   // key = "yyyy-MM-dd" local day stamp
        public static let listenedAudio = "listenedAudio"   // key = audio id
        public static let counter = "counter"               // key = "<counterKey>|<deviceId>"
        public static let taskCompletion = "taskCompletion" // key = "<dayStamp>|<taskId>" (bonus checklist tasks)

        public static let all: Set<String> = [dayCompletion, listenedAudio, counter, taskCompletion]
    }
    ...
    public static let dataDidChange = Notification.Name("ProgressSyncStoreDataDidChange")
    ...
    public static let syncedCounterKeys = [
    ...
    // `internal` (not `private`) so the two convenience constructors below
    // stay callable — but not exposed publicly, so the app keeps going
    // through `.shared`.
    ...
    public func recordEvent(kind: String, key: String, payload: Data? = nil, completion: ((Bool) -> Void)? = nil) {
    ...
    public func recordEvents(kind: String, items: [(key: String, payload: Data?)], completion: ((Bool) -> Void)? = nil) {
    ...
    public func deleteEvent(kind: String, key: String, completion: ((Bool) -> Void)? = nil) {
        context.perform { [weak self] in
            guard let self = self else {
                DispatchQueue.main.async { completion?(false) }
                return
    ...
    public struct Event {
        ...
        public init(key: String, payload: Data?, createdAt: Date) {
            self.key = key
    ...
    public func events(ofKind kind: String, completion: @escaping ([Event]) -> Void) {
        context.perform { [weak self] in
            guard let self = self else {
    ...
    /// Same "yyyy-MM-dd" formatter used by `BurstCompletionTracker.dayStamp`
    /// in the app target. Inlined so this file does not import back through
    /// the app; the format must not drift from that call site.
    ...
    private func pruneStaleTaskCompletions() {
        ...
        let cutoffStamp = Self.dayStampFormatter.string(from: cutoffDate)
        context.perform { [weak self] in
            guard let self = self else { return }
            do {
                let request = ProgressEventEntry.fetchRequest()
