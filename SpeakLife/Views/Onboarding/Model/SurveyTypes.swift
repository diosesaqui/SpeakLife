...
//  Each enum maps user responses to personalized app content.
//

import Foundation

// MARK: - Survey Step Enum

enum SurveyStep: Int, CaseIterable {
    case intro              = 0
    case heaviestBurden     = 1
    case burdenDuration     = 2
    case interstitialA      = 3   // "You're not alone"
    case failedAttempts     = 4
    case innerLie           = 5
    case interstitialB      = 6   // The pivot — introduces declaration solution
    ...
    // Progress index among question screens only (for progress bar)
...
class SurveyResponses: ObservableObject {
    ...
    /// Final goal word — uses explicit selection or derives from burden
