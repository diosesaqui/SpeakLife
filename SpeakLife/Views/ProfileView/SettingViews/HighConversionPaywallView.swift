...
/// Resolved copy model for HighConversionPaywallView —
/// bridges SurveyPersonalizationEngine output and UserPreferencesTracker fallback
struct ResolvedHCPaywallCopy {
    let headline: String
    let subheadline: String
    let valueProps: [String]
    let challengeName: String?   // nil when no survey data
}
...
struct HighConversionPaywallView: View {
    ...
    /// Survey personalization engine — takes priority over category-based copy
    ...
    private var resolvedCopy: ResolvedHCPaywallCopy {
        if surveyEngine.hasSurveyData {
            let s = surveyEngine.paywallCopy
            return ResolvedHCPaywallCopy(
                headline: s.headline,
                subheadline: s.subheadline,
                valueProps: s.valueProps.map { $0.title },
    ...
    // Legacy — kept for any remaining references; resolvedCopy is preferred
    ...
    private var benefitsSection: some View {
        ...
        let props = Array(resolvedCopy.valueProps.prefix(4))
    ...
    private var ctaText: String {
        ...
        if isEligibleForTrial {
            // Use personalized challenge name if available, otherwise generic
            if let challenge = resolvedCopy.challengeName {
                return "Start My \(challenge)"
            }
