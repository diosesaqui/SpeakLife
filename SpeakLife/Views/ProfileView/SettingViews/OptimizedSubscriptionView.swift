...
/// Unified copy model — resolved from survey engine or category-based fallback
struct ResolvedPaywallCopy {
    let headline: String
    let subheadline: String
    let ctaText: String
    let urgencyText: String
    let valueProps: [AbideStyleBenefit]
}
...
struct AbideStyleCTAButton: View {
    ...
    /// Legacy init — fetches copy from UserPreferencesTracker for non-survey contexts
    init(action: @escaping () -> Void) {
        self.title = UserPreferencesTracker.shared.getDynamicPaywallCopy().ctaText
        self.action = action
    }

    /// Personalized init — accepts explicit title from SurveyPersonalizationEngine
...
struct OptimizedSubscriptionView: View {
    ...
    /// Survey-driven personalization engine — takes priority over category-based copy
    private var surveyEngine: SurveyPersonalizationEngine {
        SurveyPersonalizationEngine(goalWordRaw: appState.surveyGoalWord)
    }

    /// Resolved paywall copy: survey goal word first, category fallback second
    private var resolvedPaywallCopy: ResolvedPaywallCopy {
        if surveyEngine.hasSurveyData {
            let s = surveyEngine.paywallCopy
            return ResolvedPaywallCopy(
                headline: s.headline,
                subheadline: s.subheadline,
                ctaText: s.ctaText,
                urgencyText: s.urgencyText,
                valueProps: s.valueProps.map { AbideStyleBenefit(icon: $0.icon, title: $0.title, description: $0.description) }
            )
        } else {
            let p = preferencesTracker.getDynamicPaywallCopy()
            return ResolvedPaywallCopy(
                headline: p.headline,
                subheadline: p.subheadline,
                ctaText: p.ctaText,
                urgencyText: p.urgencyText ?? "3 Days Free • Cancel Anytime",
                valueProps: getDefaultBenefits()
            )
        }
    }
    ...
    private var benefits: [AbideStyleBenefit] {
        if surveyEngine.hasSurveyData {
            return surveyEngine.paywallCopy.valueProps.map {
                AbideStyleBenefit(icon: $0.icon, title: $0.title, description: $0.description)
            }
        }
        ...
        return ZStack {
            ...
            VStack(spacing: iPad ? 20 : geometry.size.height * 0.02) {
                ...
                VStack(spacing: iPad ? 6 : geometry.size.height * 0.008) {
                    Text(resolvedPaywallCopy.headline)
                        .font(.system(size: joinTextSize, weight: .semibold))
                        .foregroundColor(.white)
                        .multilineTextAlignment(.center)
                        .lineLimit(iPad ? 1 : 2)

                    Text(resolvedPaywallCopy.subheadline)
...
//            )
    ...
    private var ctaButton: some View {
        AbideStyleCTAButton(title: resolvedPaywallCopy.ctaText, action: makePurchase)
            .frame(maxWidth: .infinity)
    }
