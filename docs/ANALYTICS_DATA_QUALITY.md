...
| `onboarding_variant_assigned` | **Changed meaning at the storm-arm build.** Before: fired when an ad link arrived (~2% of installs), `source` = deeplink/ad/branch. After: fires once per install when onboarding locks the arm, `source` = debug/ad/ad_storm/remote_config/legacy_default. The old meaning is now `onboarding_ad_link_received`. Any filter on the old `source` values goes to zero from that build. |
| `purchase_cancelled` | **Changed meaning at the storm-arm build.** Before: "no entitlement after the purchase call", which lumped real sheet cancels with entitlement misconfigurations (Ask-to-Buy threw and fired nothing). After: only RevenueCat's `userCancelled`. Split out: `purchase_pending` (Ask-to-Buy), `purchase_failed` (thrown error, with `error`), `purchase_no_entitlement`. New denominator: `purchase_sheet_opened`, fired right before Apple's sheet. All carry `product_id`, `paywall_name`, `trial_eligible`, `variant`. Sheet completion = trial/purchase ÷ `purchase_sheet_opened`. |
| `used_audio_in_trial` / `used_chat_in_trial` | Person properties, set false when a trial starts and true on the first audio play / Bible chat message inside the trial window. Trials started before the storm-arm build have neither; filter to trials from that build on. |
