/**
 * bibleChatPersonal.js
 * Firebase Cloud Function (v2 HTTPS) — Bible Chat AI proxy, profile-aware.
 *
 * A SEPARATE function from `bibleChat`, deliberately. bibleChat.js is live in
 * production serving paying users, so it is not edited: this is a fork that
 * adds SoulProfile context, deployed alongside it under its own name. The
 * client chooses between them at runtime (Remote Config `bibleChatPersonalized`),
 * which makes the rollout a flag flip and the rollback instant, with no
 * redeploy of the path that already works.
 *
 * Both functions meter into the SAME `bibleChatUsage` document, so a user's
 * free-message count and token totals stay correct however they are routed and
 * whichever way the flag moves.
 *
 * Retire path: once this has run clean for a while, bibleChat.js can be
 * deleted and this renamed — one deploy, no behavior change.
 *
...
// ═══════════════════════════════════════════════════════════════════════════
exports.bibleChatPersonal = onRequest(
  {
    region: 'us-central1',
    cors: true,
